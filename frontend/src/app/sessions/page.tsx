    ...
    <div className="min-h-screen bg-[#fafafa] dark:bg-[#141411]">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        {liveRunning.length > 0 && (
          <div className="mb-6 rounded-[20px] border border-[rgba(14,15,12,0.08)] dark:border-[rgba(232,235,230,0.08)] bg-white dark:bg-[#1e201c] p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="ml-auto text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[rgba(159,232,112,0.12)] text-[#054d28] dark:text-[#9fe870]">{liveRunning.length} aktif</span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {liveRunning.map(s => {
                const pnl = livePnlBySession[s.id]
                const isIDR = s.symbol.endsWith('_IDR')
                const isBlocked = blockedDca.some(item => item.id === s.id)
                const accent = s.strategy === 'grid'
                  ? 'border-l-[#9fe870]'
                  : s.strategy === 'trend'
                    ? 'border-l-[#38c8ff]'
                    : 'border-l-[#ffd11a]'
                return (
                  <button key={s.id} onClick={() => router.push(`/sessions/${s.id}`)}
                    className={`text-left px-3 py-2.5 rounded-[14px] border border-l-[3px] ${accent} border-[rgba(14,15,12,0.08)] dark:border-[rgba(232,235,230,0.08)] bg-[#fafafa] dark:bg-[#252822] hover:bg-white dark:hover:bg-[#2a2c27] transition-colors`}>
                    <div className="flex items-center gap-1.5 mb-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#9fe870] animate-pulse flex-shrink-0" />
                      <span className="text-[10px] font-bold text-[#0e0f0c] dark:text-[#e8ebe6] truncate">{s.name}</span>
                    </div>
                    <div className="flex items-center gap-1.5 mb-1 min-w-0">
                      <p className="text-[9px] text-[#686868] dark:text-[#a5a8a2] truncate">
                        {s.symbol.replace('_', '/')} · <span className="capitalize">{s.strategy}</span>
                      </p>
                      <span className={`ml-auto flex-shrink-0 text-[8px] font-bold px-1.5 py-0.5 rounded-full ${isBlocked ? 'bg-[rgba(208,50,56,0.1)] text-[#d03238] dark:text-[#ff6b6f]' : 'bg-[rgba(159,232,112,0.12)] text-[#054d28] dark:text-[#9fe870]'}`}>
                        {isBlocked ? 'Tertahan saldo' : 'Aktif'}
                      </span>
                    </div>
                    {pnl != null ? (
