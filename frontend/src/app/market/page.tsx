  ...
  if (!data) {
    return (
      <div className="bg-gray-900 rounded-xl p-5 border border-gray-800">
        ...
        <p className="text-sm text-gray-500 mt-4">Mengambil harga...</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-gray-900 rounded-xl p-5 border border-gray-800 hover:border-gray-700 transition">
      ...
      <div className="mt-4">
        ...
        {approxIDR && (
          <p className="text-sm text-gray-400 mt-1">~ {formatIDR(approxIDR)}</p>
        )}
      </div>

      <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-gray-800 text-xs text-gray-500">
        <div>
          <p>24h High</p>
          <p className="text-gray-300 mt-1">{isIDRPair ? formatIDR(parseFloat(data.high24h)) : `$${formatUSDTPrice(parseFloat(data.high24h))}`}</p>
        </div>
        <div>
          <p>24h Low</p>
          <p className="text-gray-300 mt-1">{isIDRPair ? formatIDR(parseFloat(data.low24h)) : `$${formatUSDTPrice(parseFloat(data.low24h))}`}</p>
        </div>
        <div>
          <p>Volume</p>
          <p className="text-gray-300 mt-1">{parseFloat(data.volume).toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      <div className="bg-gray-900 rounded-xl p-4 mb-6 text-sm text-gray-400">
        Halaman ini menampilkan harga pair utama di TokoCrypto. Untuk pair USDT, harga juga dikonversi kira-kira ke rupiah memakai kurs <span className="text-gray-300 font-medium">USDT/IDR</span>{usdtIdrRate ? ` (${formatIDR(usdtIdrRate)})` : ''}.
      </div>
