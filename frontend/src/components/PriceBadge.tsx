...
export function PriceBadge({ symbol, compact }: { symbol: string; compact?: boolean }) {
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-4">
      {!connected && !data ? (
        <p className="text-sm text-gray-500">⏳ Menghubungkan ke TokoCrypto untuk harga {symbol}...</p>
      ) : timeout && !data ? (
        <p className="text-sm text-yellow-500">
          ⚠️ Tidak dapat terhubung ke TokoCrypto. Harga real-time tidak tersedia.{' '}
          <span className="text-gray-500">Coba refresh halaman.</span>
        </p>
      ) : !data ? (
        <p className="text-sm text-gray-500">⏳ Menunggu data harga {symbol}...</p>
      ) : (
        <>
          <div className="flex items-baseline justify-between">
            <div>
              <p className="text-xs text-gray-500 mb-1">{symbol}</p>
              <p className={`text-2xl font-bold font-mono ${parseFloat(data.priceChange) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                {parseFloat(data.lastPrice).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 8 })}
              </p>
            </div>
            <div className={`text-right ${parseFloat(data.priceChange) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      {connected && !data && <p className="text-xs text-gray-500 mt-2">Terhubung, menunggu data...</p>}
    </div>
  )
}
