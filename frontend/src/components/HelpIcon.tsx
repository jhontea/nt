...
export function HelpIcon({ text }: { text: string }) {
  ...
  return (
    <span className="relative inline-block ml-1">
      ...
      >?</span>
      {open && (
        <span id={tooltipId} role="tooltip" className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-[#0e0f0c] dark:bg-[#e8ebe6] text-[#e8ebe6] dark:text-[#0e0f0c] text-xs rounded-[10px] w-max max-w-[min(224px,80vw)] shadow-lg whitespace-pre-wrap pointer-events-none">
          {text}
        </span>
      )}
    </span>
