...
var ErrLiveOrderSkipped = errors.New("live order was not executed")
...
func (l *LiveEngine) Execute(session model.Session, signal Signal) error {
	...
	if err := l.db.Get(&recentCount, l.db.Rebind(
		...
		return fmt.Errorf("%w: duplicate order suppressed", ErrLiveOrderSkipped)
	}
	...
	if signal.Side == string(model.SideSell) {
		...
		if notional < minNotional {
			...
			return fmt.Errorf("%w: sell notional %.8f below minimum %.8f", ErrLiveOrderSkipped, notional, minNotional)
		}
	}
